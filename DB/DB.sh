...
create event prune_data on schedule every 1 day do delete from data where 2592000 <= UNIX_TIMESTAMP(DATE);
create database Switch_Conf;
use Switch_Conf;
create table conf (ID int, CONF TEXT(65535), DATE date);
create database QoS_Log;
use QoS_Log;
create table logs (ID int, PROBLEM varchar(255), SOLUTION varchar(255), DATE date);
