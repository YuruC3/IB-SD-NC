...
import requests
import datetime
from SDN.Ansible.sendmail import *
from SDN.Ansible.noIpPlease import *
from SDN.Ansible.whatTheConfig import *
...
def analyze_netflow_data(analysis:dict, new_data:dict):
    ...
    analysis['packets'][analysis['total_packets_analyzed']] = new_data
    print ("added: ", new_data, "to analysis")
    if new_data['src_ip'] not in analysis['unique_src_ips']:
        analysis['unique_src_ips'][new_data['src_ip']] = 1
    else:
        analysis['unique_src_ips'][new_data['src_ip']] += 1
    if new_data['dst_port'] not in analysis['unique_dst_ports']:
        analysis['unique_dst_ports'][new_data['dst_port']] = 1
    else:
        analysis['unique_dst_ports'][new_data['dst_port']] += 1
    for keys in analysis['unique_src_ips']:
        if True:#analysis['unique_src_ips'][keys] >= 5:
